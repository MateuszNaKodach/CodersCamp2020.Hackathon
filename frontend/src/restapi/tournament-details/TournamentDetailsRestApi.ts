...
export const TournamentDetailsRestApi = (config?: Partial<TournamentDetailsRestApiConfig>) => {
  const currentConfig = {
    ...defaultConfig,
    config,
  ...
  return {
    getAllTournamentsDetails(): Promise<TournamentDetailsListDto> {
      return axios.get<TournamentDetailsListDto>(`${currentConfig.baseUrl}/tournament-details/`).then((res) => res.data);
    },
    async addTournamentName(body: { tournamentId: string; tournamentName: string }): Promise<void> {
