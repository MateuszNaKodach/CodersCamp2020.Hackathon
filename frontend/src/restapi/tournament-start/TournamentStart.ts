...
export const TournamentStartRestApi = (config?: Partial<TournamentStartRestApiConfig>) => {
  const currentConfig = {
    ...defaultConfig,
    config,
  ...
  return {
    async startTournament(tournamentId: string): Promise<void> {
      await axios.post(`${currentConfig.baseUrl}/doubles-tournaments/${tournamentId}/start`, null, { params: { tournamentId } });
    },
  };
};
