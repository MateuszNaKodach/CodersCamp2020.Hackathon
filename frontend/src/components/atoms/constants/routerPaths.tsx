// export const PATH_FOR_HOME_VIEW: string = "/home";
export const PATH_FOR_LOGIN_VIEW: string = "/";
// export const PATH_FOR_TOURNAMENTS_SELECTION_VIEW: string =
//   "/tournaments-selection";
// export const PATH_FOR_CREATING_NEW_TOURNAMENT_VIEW: string = "/new-tournament";
// export const PATH_FOR_TOURNAMENT_REGISTRATIONS_VIEW: string =
//   "/tournament-registration";
// export const PATH_FOR_TOURNAMENT_REGISTRATIONS_VIEW_WITHOUT_ID: string =
//   PATH_FOR_TOURNAMENT_REGISTRATIONS_VIEW + "/:tournamentId";
// export const PATH_FOR_PLAYERS_PROFILES_VIEW: string = "/players-profiles";
// export const PATH_FOR_PLAYER_PROFILE_CREATION_VIEW: string =
//   "/new-player-profiles";
// export const PATH_FOR_PLAYER_MATCHES_VIEW: string =
//   "/tournament/:tournamentId/matches";
// export const PATH_FOR_TOURNAMENT: string = "/tournament";
