...
export const UserProfileRestApi = (config?: Partial<UserProfilesRestApiConfig>) => {
  const currentConfig = {
    ...defaultConfig,
    config,
  ...
  return {
    getPlayersProfiles(): Promise<PlayerProfilesListDto> {
      return axios.get<PlayerProfilesListDto>(`${currentConfig.baseUrl}/players-profiles`).then((response) => response.data);
    },

    getPlayerProfile(playerId: string): Promise<PlayerProfileDto> {
      return axios.get<PlayerProfileDto>(`${currentConfig.baseUrl}/players-profiles/${playerId}`).then((response) => response.data);
    },

    async postPlayersProfile(playerProfile: PlayerProfileDto): Promise<void> {
      await axios.post<PlayerProfileDto>(`${currentConfig.baseUrl}/players-profiles`, playerProfile);
    },
  };
};
