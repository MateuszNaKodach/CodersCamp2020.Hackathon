...
export const TournamentTablesRestApi = (config?: Partial<TournamentTablesRestApiConfig>) => {
  const currentConfig = {
    ...defaultConfig,
    config,
  ...
  return {
    async addTournamentsTables(params: { tournamentId: string; tablesBody: TablesList }): Promise<void> {
      await axios.post(`${currentConfig.baseUrl}/tournaments/${params.tournamentId}/tables`, params.tablesBody, {
