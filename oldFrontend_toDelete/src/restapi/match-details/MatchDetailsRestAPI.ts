...
export const MatchDetailsRestAPI = (config?: Partial<MatchDetailsRestApiConfig>) => {
  const currentConfig = {
    ...defaultConfig,
    config,
  ...
  return {
    getTournamentMatch(matchId: string): Promise<MatchDetailsDto> {
      return axios.get<MatchDetailsDto>(`${currentConfig.baseUrl}/matches/${matchId}`).then((res) => res.data);
    },

    async postMatchWinner(matchId: string, winnerPlayerId: string): Promise<void> {
